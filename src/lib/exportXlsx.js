...
import { cost, amount, expensesTotal, hoursBetween, fmtDateTime } from './utils'

export function exportXlsx(completed, totals) {
  const aoa = [['Tracker Report'], [`Generated ${new Date().toLocaleString()}`], [],
    ['Client', 'Start', 'End', 'Hours', 'Rate/hr', 'Time $', 'Expenses $', 'Expense detail', 'Amount', 'Paid', 'Note']]
  completed.slice().reverse().forEach((r) => {
    const h = hoursBetween(r.start_ts, r.end_ts)
    const detail = (r.expenses || []).map((e) => `${e.type} ${Number(e.amount).toFixed(2)}`).join(', ')
    aoa.push([r.renter, fmtDateTime(r.start_ts), fmtDateTime(r.end_ts),
      ...
      Number(expensesTotal(r).toFixed(2)), detail, Number(amount(r).toFixed(2)),
      r.paid ? 'Yes' : 'No', r.note || ''])
  })
  const blank = ['', '', '', '', '', '', '', '', '', '', '']
  aoa.push(blank)
  aoa.push(['', '', 'TOTAL', Number(totals.hrs.toFixed(2)), '', '', Number(totals.expenses.toFixed(2)), '', Number(totals.earned.toFixed(2)), '', ''])
  aoa.push(['', '', 'PAID', '', '', '', '', '', Number(totals.paid.toFixed(2)), '', ''])
  aoa.push(['', '', 'OUTSTANDING', '', '', '', '', '', Number(totals.due.toFixed(2)), '', ''])
  const ws = XLSX.utils.aoa_to_sheet(aoa)
  ws['!cols'] = [{ wch: 16 }, { wch: 18 }, { wch: 18 }, { wch: 8 }, { wch: 9 }, { wch: 9 }, { wch: 11 }, { wch: 24 }, { wch: 10 }, { wch: 6 }, { wch: 24 }]
